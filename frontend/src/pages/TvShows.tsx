...
export const TvShows: React.FC = () => {
    ...
    useEffect(() => {
        ...
        setCurrentPage(1);
        setHasMore(true);
        loadSeries(1, true, { genre: urlGenre, search: urlSearch, year: urlYear });
    }, [searchParams]);

    // Initial Load & Genres
    useEffect(() => {
        const init = async () => {
            await Promise.all([loadGenres(), loadYears(), loadRecommendations()]);
        };
        init();
    }, []);
    ...
    useEffect(() => {
        if (loading && !searchParams.get('genre') && !searchParams.get('search') && !searchParams.get('q')) return;
    ...
    const loadGenres = async () => {
        try {
            ...
            const filteredGenres = genresData.filter(g =>
                !["Thriller", "TV Movie", "Science Fiction", "Horror"].includes(g)
            );
    ...
    const loadSeries = async (page: number, isReset: boolean = false, overrides?: { search?: string, genre?: string, year?: string }) => {
        if (isReset) {
            setLoading(true);
        } else {
            setLoadingMore(true);
        ...
        try {
            ...
            const data = await apiService.getMovies({
                ...
                ordering: (currentSearch || currentGenre || currentYear) ? '-year,-imdb_id' : 'random'
            });
    ...
    return (
        <div className="min-h-screen bg-dark-900 pb-20">
            {/* Hero Section */}
            {!searchQuery && !genreFilter && !yearFilter && featuredSeries && <FeaturedMovieHero movie={featuredSeries} />}
            ...
            {/* Content Grid */}
            <div className={`px-6 md:px-12 space-y-8 ${!searchQuery && !genreFilter && !yearFilter ? 'mt-4' : 'pt-10'}`}>
                {/* Recommended Carousel */}
                {!searchQuery && !genreFilter && !yearFilter && recommendedSeries.length > 0 && (
                ...
                <div className="flex items-center justify-between border-b border-white/10 pb-2">
                    <h3 className="text-xl font-semibold text-gray-200 uppercase tracking-wider">
                        {searchQuery || genreFilter || yearFilter ? 'Search Results' : 'Explore All TV Shows'}
                    </h3>
                    {series.length > 0 && <span className="text-sm text-gray-500 font-bold">{totalCount} Total Items</span>}
                </div>
                    ...
                    <>
                        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-x-4 gap-y-10 animate-fade-in">
                            {series.map((show, index) => (
                                <MovieCard key={`${show.imdb_id}-${index}`} movie={show} />
                            ))}
                        </div>
